...
uint16_t FIRE_RATE_15MS_SET = 4220; //速度上限15
uint16_t FIRE_RATE_18MS_SET = 4600; //速度上限18
...
int32_t TRI_SINGLE_SPEED_SET = 8000; //单环拨轮转速设置
#define NINE_TRI_ANGLE_SET 52.0f     //拨弹论单发角度
//遥控输入模式设定
...
static void shoot_init(Shoot_Motor_t *shoot_motor_init)
{
    ...
    //获取电机指针
    shoot_motor_init->trigger_motor_measure = get_Trigger_Motor_Measure_Point();     //获得电机数据
    shoot_motor_init->trigger_encoder_measure = get_Trigger_Encoder_Measure_Point(); //获得电机编码器数据
    ...
    //初始化PID
    PID_Init(&shoot_motor_init->trigger_motor_single_pid, PID_POSITION, Trigger_single_pid, TRIGGER_SINGLE_PID_MAX_OUT, TRIGGER_SINGLE_PID_MAX_IOUT,0,0,0,0,0);
    PID_Init(&shoot_motor_init->friction_motor_left_pid, PID_POSITION, friction_left_pid, FRICTION_PID_MAX_OUT, FRICTION_PID_MAX_IOUT,0,0,0,0,0);
    PID_Init(&shoot_motor_init->friction_motor_right_pid, PID_POSITION, friction_right_pid, FRICTION_PID_MAX_OUT, FRICTION_PID_MAX_IOUT,0,0,0,0,0);
    //初始化双环控制PID
    PID_Init(&shoot_motor_init->trigger_angle_bicyclo_pid, PID_POSITION, Trigger_angle_pid, TRIGGER_ANGLE_PID_MAX_OUT, TRIGGER_ANGLE_PID_MAX_IOUT,0,0,0,0,0);
    PID_Init(&shoot_motor_init->trigger_speed_bicyclo_pid, PID_POSITION, Trigger_speed_pid, TRIGGER_SPEED_PID_MAX_OUT, TRIGGER_SPEED_PID_MAX_IOUT,0,0,0,0,0);
}
/**
 * @brief          射击数据更新
 * @author         RM
    ...
    */
    ...
    if (*inputmode == KEYMOUSE_INPUT)
    {
        ...
        if (EFlag_state)
        {
            ...
            {
                if (shoot_mode->shoot_status_measure->shooter_id1_17mm_speed_limit - shoot_mode->shoot_data_measure->bullet_speed <= 0.3f && last_bullet_speed != shoot_mode->shoot_data_measure->bullet_speed)
                {
        ...
        if (shoot_mode->shoot_rc_ctrl->mouse.press_l == 1)
        {
            ...
            if (MOU_LEFT_state <= 100)
            {
                Repeater_flag = 1;
            }
            else if (MOU_LEFT_state > 100)
            {
                Repeater_flag = 2;
            }
        }
    ...
    if (*inputmode == REMOTE_INPUT)
    {
        ...
        if (shoot_mode->shoot_rc_ctrl->rc.ch[4] > 0)
        {
            ...
            {
                shoot_mode->trigger_motor_set_speed = 8000;
                shoot_ctrl_limit(shoot_mode);
            }
