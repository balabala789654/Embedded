...
float yaw_relative_last,yaw_relative;
int yaw_count;
...
extern AGV_chassis_t agv_chassis; //舵轮底盘相关结构体
//遥控输入模式设定
...
static void GIMBAL_Init(Gimbal_Control_t *gimbal_init)
{
    ...
    // yaw角度环
    GIMBAL_PID_Init(&gimbal_init->gimbal_yaw_motor.gimbal_motor_angle_pid, YAW_PID_MAX_OUT, YAW_PID_MAX_IOUT, YAW_INIT_PID_KP, YAW_INIT_PID_KI, YAW_INIT_PID_KD, 0.001, 0.5, 0, 0, 0);
    // yaw速度环
    PID_Init(&gimbal_init->gimbal_yaw_motor.gimbal_motor_gyro_pid, PID_POSITION, Yaw_speed_pid, YAW_SPEED_PID_MAX_OUT, YAW_SPEED_PID_MAX_IOUT, 0, 0, 0, 0, 0);
    // pit角度环
    GIMBAL_PID_Init(&gimbal_init->gimbal_pitch_motor.gimbal_motor_angle_pid, PITCH_PID_MAX_OUT, PITCH_PID_MAX_IOUT, PITCH_INIT_PID_KP, PITCH_INIT_PID_KI, PITCH_INIT_PID_KD, 0, 0, 0, 0, 0);
    // pit速度环
    PID_Init(&gimbal_init->gimbal_pitch_motor.gimbal_motor_gyro_pid, PID_POSITION, Pitch_speed_pid, PITCH_SPEED_PID_MAX_OUT, PITCH_SPEED_PID_MAX_IOUT, 0, 0, 0, 0, 0);
}
//底盘无力模式
...
static void gimbal_track_armor(Gimbal_Control_t *gimbal_motor)
{
    ...
    if (gimbal_motor->gimbal_monitor_point[TX2DataWGD].errorExist == 1) // TX2离线
    {
        ...
        gimbal_motor->gimbal_pitch_motor.gimbal_angle_set -= PITCH_TURN * (gimbal_motor->gimbal_rc_ctrl->rc.ch[PitchChannel] * STICK_TO_PITCH_ANGLE_INC_FACT + fp32_constrain(gimbal_motor->gimbal_rc_ctrl->mouse.y, -45, +45) * MOUSE_TO_PITCH_ANGLE_INC_FACT);
        gimbal_motor->gimbal_yaw_motor.gimbal_angle_set += (-gimbal_motor->gimbal_rc_ctrl->rc.ch[YawChannel] * STICK_TO_YAW_ANGLE_INC_FACT) - (fp32_constrain(gimbal_motor->gimbal_rc_ctrl->mouse.x, -45, +45) * MOUSE_TO_YAW_ANGLE_INC_FACT);
...
static void gimbal_shoot_buff(Gimbal_Control_t *gimbal_motor)
{
    ...
    if (gimbal_motor->gimbal_monitor_point[TX2DataWGD].errorExist == 0)
    {
        ...
        gimbal_motor->gimbal_pitch_motor.gimbal_angle_set -= PITCH_TURN * (gimbal_motor->pitch_angle_PcCtrl_ref * pitch_rate + gimbal_motor->gimbal_rc_ctrl->rc.ch[PitchChannel] * STICK_TO_PITCH_ANGLE_INC_FACT + fp32_constrain(gimbal_motor->gimbal_rc_ctrl->mouse.y, -45, +45) * MOUSE_TO_PITCH_ANGLE_INC_FACT);

        gimbal_motor->gimbal_yaw_motor.gimbal_angle_set += gimbal_motor->yaw_angle_PcCtrl_ref * yaw_rate - gimbal_motor->gimbal_rc_ctrl->rc.ch[YawChannel] * STICK_TO_YAW_ANGLE_INC_FACT - fp32_constrain(gimbal_motor->gimbal_rc_ctrl->mouse.x, -45, +45) * MOUSE_TO_YAW_ANGLE_INC_FACT;
    }
...
{
    gimbal_pid_reset(&gimbal_motor->gimbal_yaw_motor.gimbal_motor_angle_pid, YAW_INIT_PID_KP, YAW_INIT_PID_KI, YAW_INIT_PID_KD, 0, 0, 0, 0, 0);
    pid_reset(&gimbal_motor->gimbal_yaw_motor.gimbal_motor_gyro_pid, YAW_SPEED_PID_KP, YAW_SPEED_PID_KI, YAW_SPEED_PID_KD, 0, 0, 0, 0, 0);

    gimbal_pid_reset(&gimbal_motor->gimbal_pitch_motor.gimbal_motor_angle_pid, PITCH_INIT_PID_KP, PITCH_INIT_PID_KI, PITCH_INIT_PID_KD, 0, 0, 0, 0, 0);
    pid_reset(&gimbal_motor->gimbal_pitch_motor.gimbal_motor_gyro_pid, PITCH_SPEED_PID_KP, PITCH_SPEED_PID_KI, PITCH_SPEED_PID_KD, 0, 0, 0, 0, 0);
...
static void gimbal_turn_180(Gimbal_Control_t *gimbal_motor)
{
    ...
    gimbal_motor->gimbal_pitch_motor.gimbal_angle_set = gimbal_motor->gimbal_angle_gyro_point->ROLL;
    gimbal_absolute_angle_control(&gimbal_motor->gimbal_pitch_motor);
...
static void gimbal_keymouse_scan(Gimbal_Control_t *gimbal_keymouse, remote_mode_e *inputmode)
{
    ...
    if (*inputmode == KEYMOUSE_INPUT)
    {
        ...
        if (gimbal_keymouse->gimbal_rc_ctrl->key.v & KEY_PRESSED_OFFSET_Z && Z_Flag == 1)
        {
            ...
            CFlag_state = 0;
            XFlag_state = 0;
            gimbal_keymouse->gimbal_yaw_motor.gimbal_turn_around = gimbal_keymouse->gimbal_yaw_motor.absolute_angle + 180;
        }
        /********按下X->左旋90度************************************************************************************************************/
...
static void gimbal_behavour_set(Gimbal_Control_t *gimbal_mode_set, remote_mode_e *inputmode)
{
    ...
    if (*inputmode == RUN_STOP) //(双下)停止指令
    {
        ...
        if (switch_is_down(gimbal_mode_set->gimbal_rc_ctrl->rc.s[ModeChannel_R]) && switch_is_up(gimbal_mode_set->gimbal_rc_ctrl->rc.s[ModeChannel_L]))
        {
            //左上右下
...
;
double p_angle_KP = 23, p_angle_KI = 0, p_angle_KD = 0, p_speed_KP = 115;
;
/*****自瞄PID**************/
float y_zm_anglep = 30, y_zm_anglei = 0, y_zm_angled = 0, y_zm_speedp = 160;
float p_zm_anglep = 10, p_zm_anglei = 0, p_zm_angled = 0, p_zm_speedP = 140;
/*****能量机关************/
float y_sf_anglep = 35, y_sf_anglei = 0, y_sf_angled = 0, y_sf_speedp = 150;
float p_sf_anglep = 20, p_sf_anglei = 0, p_sf_angled = 0, p_sf_speedP = 120;
static void GIMBAL_Mode_Change_Control_Transit(Gimbal_Control_t *gimbal_mode_change)
{
    ...
    {
        //初始化
        gimbal_pid_reset(&gimbal_mode_change->gimbal_yaw_motor.gimbal_motor_angle_pid, yaw_angle_p, YAW_INIT_PID_KI, YAW_INIT_PID_KD, 0, 0, 0, 0, 0);
        pid_reset(&gimbal_mode_change->gimbal_yaw_motor.gimbal_motor_gyro_pid, yaw_speed_p, YAW_SPEED_PID_KI, YAW_SPEED_PID_KD, 0, 0, 0, 0, 0);

        gimbal_pid_reset(&gimbal_mode_change->gimbal_pitch_motor.gimbal_motor_angle_pid, p_angle_p, P_angle_i, PITCH_INIT_PID_KD, 0, 0, 0, 0, 0);
        pid_reset(&gimbal_mode_change->gimbal_pitch_motor.gimbal_motor_gyro_pid, p_speed_p, PITCH_SPEED_PID_KI, PITCH_SPEED_PID_KD, 0, 0, 0, 0, 0);
    ...
    {
        //手控
        gimbal_pid_reset(&gimbal_mode_change->gimbal_yaw_motor.gimbal_motor_angle_pid, yaw_angle_KP, yaw_angle_KI, yaw_angle_KD, 0, 0, 0, 0, 0);
        pid_reset(&gimbal_mode_change->gimbal_yaw_motor.gimbal_motor_gyro_pid, yaw_speed_KP, YAW_MANUAL_SPEED_PID_KI, YAW_MANUAL_SPEED_PID_KD, 0, 0, 0, 0, 0);
        gimbal_pid_reset(&gimbal_mode_change->gimbal_pitch_motor.gimbal_motor_angle_pid, p_angle_KP, p_angle_KI, p_angle_KD, 0, 0, 0, 0, 2);
        pid_reset(&gimbal_mode_change->gimbal_pitch_motor.gimbal_motor_gyro_pid, p_speed_KP, PITCH_MANUAL_SPEED_PID_KI, PITCH_MANUAL_SPEED_PID_KD, 0, 0, 0, 0, 0);
    ...
    {
        //自瞄
        gimbal_pid_reset(&gimbal_mode_change->gimbal_yaw_motor.gimbal_motor_angle_pid, y_zm_anglep, y_zm_anglei, y_zm_angled, 0, 0, 0, 0, 0);
        pid_reset(&gimbal_mode_change->gimbal_yaw_motor.gimbal_motor_gyro_pid, y_zm_speedp, YAW_TRACK_SPEED_PID_KI, YAW_TRACK_SPEED_PID_KD, 0, 0, 0, 0, 0);
        gimbal_pid_reset(&gimbal_mode_change->gimbal_pitch_motor.gimbal_motor_angle_pid, p_zm_anglep, p_zm_anglei, p_zm_angled, 0, 0, 0, 0, 0);
        pid_reset(&gimbal_mode_change->gimbal_pitch_motor.gimbal_motor_gyro_pid, p_zm_speedP, PITCH_TRACK_SPEED_PID_KI, PITCH_TRACK_SPEED_PID_KD, 0, 0, 0, 0, 0);
    ...
    else if (gimbal_mode_change->last_gimbal_behaviour != GIMBAL_SHOOT_BUFF && gimbal_mode_change->gimbal_behaviour == GIMBAL_SHOOT_BUFF)
    {
        //射符
        gimbal_mode_change->gimbal_yaw_motor.gimbal_for_angle_set = 0;
        gimbal_pid_reset(&gimbal_mode_change->gimbal_yaw_motor.gimbal_motor_angle_pid, y_sf_anglep, y_sf_anglei, y_sf_angled, 0, 0, 0, 0, 0);
        pid_reset(&gimbal_mode_change->gimbal_yaw_motor.gimbal_motor_gyro_pid, y_sf_speedp, YAW_STBUF_SPEED_PID_KI, YAW_STBUF_SPEED_PID_KD, 0, 0, 0, 0, 0);
        gimbal_pid_reset(&gimbal_mode_change->gimbal_pitch_motor.gimbal_motor_angle_pid, p_sf_anglep, p_sf_anglei, p_sf_angled, 0, 0, 0, 0, 0);
        pid_reset(&gimbal_mode_change->gimbal_pitch_motor.gimbal_motor_gyro_pid, p_sf_speedP, PITCH_STBUF_SPEED_PID_KI, PITCH_STBUF_SPEED_PID_KD, 0, 0, 0, 0, 0);
...
static void GIMBAL_Feedback_Update(Gimbal_Control_t *gimbal_feedback_update)
{
    ...
    yaw_relative_last = gimbal_feedback_update->gimbal_yaw_motor.relative_angle;
		...
		if(agv_chassis.rc_mode == AGV_CHASSIS_FOLLOW_GIMBAL||agv_chassis.rc_mode == AGV_CHASSIS_KEY_MODE)
		{
		if(yaw_relative_last-gimbal_feedback_update->gimbal_yaw_motor.relative_angle>=180)
		{
			yaw_count++;
		}
		else if(yaw_relative_last-gimbal_feedback_update->gimbal_yaw_motor.relative_angle<=-180)
		{
			yaw_count--;
		}
		yaw_relative = gimbal_feedback_update->gimbal_yaw_motor.relative_angle+yaw_count*360;
	  }
    ...
    //动态输入角度更新
    gimbal_feedback_update->pitch_angle_dynamic_ref += PITCH_TURN * (gimbal_feedback_update->gimbal_rc_ctrl->rc.ch[PitchChannel] * STICK_TO_PITCH_ANGLE_INC_FACT - fp32_constrain(KalmanFilter(&KF_Mouse_Y_Speed, gimbal_feedback_update->gimbal_rc_ctrl->mouse.y), -45, +45) * MOUSE_TO_PITCH_ANGLE_INC_FACT); //此处鼠标参数负号为取反
    gimbal_feedback_update->yaw_angle_dynamic_ref -= gimbal_feedback_update->gimbal_rc_ctrl->rc.ch[YawChannel] * STICK_TO_YAW_ANGLE_INC_FACT + fp32_constrain(KalmanFilter(&KF_Mouse_X_Speed, gimbal_feedback_update->gimbal_rc_ctrl->mouse.x), -45, +45) * MOUSE_TO_YAW_ANGLE_INC_FACT;
    //限幅
